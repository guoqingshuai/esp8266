...
LOCAL os_timer_t inputTimer;                                                 ///< User timer structure
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagMotor_Spare)
    {
        info->event[info->num] = EVENT_Motor_Spare;
        info->num++;
        dataPoints->valueMotor_Spare = gizStandardDecompressionValue(Motor_Spare_BYTEOFFSET,Motor_Spare_BITOFFSET,Motor_Spare_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagFlow_clean)
    {
        info->event[info->num] = EVENT_Flow_clean;
        info->num++;
        dataPoints->valueFlow_clean = gizStandardDecompressionValue(Flow_clean_BYTEOFFSET,Flow_clean_BITOFFSET,Flow_clean_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueMotor_Spare != cur->valueMotor_Spare)
    {
        GIZWITS_LOG("valueMotor_Spare Changed\n");
        ret = 1;
    }
    if(last->valueFlow_clean != cur->valueFlow_clean)
    {
        GIZWITS_LOG("valueFlow_clean Changed\n");
        ret = 1;
    }
    if(last->valueLiquid != cur->valueLiquid)
    {
        GIZWITS_LOG("valueLiquid Changed\n");
        ret = 1;
    }
    ...
    if(last->valuecumulative_flow != cur->valuecumulative_flow)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuecumulative_flow Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Motor_Spare_BYTEOFFSET,Motor_Spare_BITOFFSET,Motor_Spare_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMotor_Spare);
    gizStandardCompressValue(Flow_clean_BYTEOFFSET,Flow_clean_BITOFFSET,Flow_clean_LEN,(uint8_t *)devStatusPtr,dataPoints->valueFlow_clean);
    if(dataPoints->valueLiquid >= Liquid_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueLiquid Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Liquid_BYTEOFFSET,Liquid_BITOFFSET,Liquid_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLiquid);  
    }
    ...
    devStatusPtr->valueSpeed = exchangeBytes(gizY2XFloat(Speed_RATIO,  Speed_ADDITION, dataPoints->valueSpeed));     

    devStatusPtr->valuecumulative_flow = exchangeWord(gizY2X(cumulative_flow_RATIO,  cumulative_flow_ADDITION, dataPoints->valuecumulative_flow)); 
...
void devAuthResultCb( int32 result )
{
    ...
    os_timer_arm(&userTimer, USER_TIME_MS, 1);//1000ms
        //user timer 
    os_timer_disarm(&inputTimer);
    os_timer_setfn(&inputTimer, (os_timer_func_t *)inputHandle, NULL);
    os_timer_arm(&inputTimer, MAX_INPOT_TIMOUT, 1);//1000ms
}
